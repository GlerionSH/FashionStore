alter table if exists public.fs_order_items
	add column if not exists paid_unit_cents integer null,
	add column if not exists paid_line_total_cents integer null;
...
with base as (
	select
		...
		i.qty,
		i.created_at,
		i.line_total_cents as base_line_total_cents,
		o.subtotal_cents,
		o.total_cents,
		case
			when o.subtotal_cents > 0 then (o.total_cents::numeric / o.subtotal_cents::numeric)
			else 1
		end as ratio,
		round(
			i.line_total_cents::numeric *
			case
				when o.subtotal_cents > 0 then (o.total_cents::numeric / o.subtotal_cents::numeric)
				else 1
			end
		)::int as provisional_paid_line_total_cents
	from public.fs_order_items i
	join public.fs_orders o on o.id = i.order_id
),
per_order as (
	select
		order_id,
		max(total_cents)::int as order_total_cents,
		sum(provisional_paid_line_total_cents)::int as sum_paid_line_total_cents
	from base
	group by order_id
),
ranked as (
	select
		b.*,
		(po.order_total_cents - po.sum_paid_line_total_cents)::int as diff_cents,
		row_number() over (partition by b.order_id order by b.created_at desc, b.id desc) as rn
	from base b
	join per_order po on po.order_id = b.order_id
)
update public.fs_order_items i
set
	paid_line_total_cents = (r.provisional_paid_line_total_cents + case when r.rn = 1 then r.diff_cents else 0 end),
	paid_unit_cents = case
		when i.qty > 0 then round(((r.provisional_paid_line_total_cents + case when r.rn = 1 then r.diff_cents else 0 end)::numeric) / i.qty)::int
		else null
	end
from ranked r
where i.id = r.id
	and (i.paid_line_total_cents is null or i.paid_unit_cents is null);

update public.fs_return_items ri
set line_total_cents = case
	when oi.qty > 0 and oi.paid_line_total_cents is not null
		then round(oi.paid_line_total_cents::numeric * (ri.qty::numeric / oi.qty::numeric))::int
	else ri.line_total_cents
end
from public.fs_order_items oi
	...
	if v_subtotal > 0 then
		with items as (
			select id, qty, created_at,
				round(line_total_cents::numeric * (v_total::numeric / v_subtotal::numeric))::int as provisional
			from public.fs_order_items
		...
		ranked as (
			select i.*, (v_total - (select sum_prov from sumprov))::int as diff,
		...
		update public.fs_order_items
		set
			paid_line_total_cents = line_total_cents,
			paid_unit_cents = case when qty > 0 then round(line_total_cents::numeric / qty)::int else null end
		where order_id = v_order_id;
	end if;
