-- FashionStore: finalize paid orders in webhook + email tracking
...
create or replace function public.fs_finalize_order_paid(
	order_id uuid,
...
begin
	select * into v_order
	from public.fs_orders o
	where o.id = fs_finalize_order_paid.order_id
	for update;
	...
	-- Update order paid fields
	v_email := nullif(trim(coalesce(fs_finalize_order_paid.customer_email, '')), '');
	...
	set
		...
		stripe_payment_intent_id = coalesce(public.fs_orders.stripe_payment_intent_id, fs_finalize_order_paid.stripe_payment_intent_id),
		stripe_session_id = coalesce(public.fs_orders.stripe_session_id, fs_finalize_order_paid.stripe_session_id)
	where id = fs_finalize_order_paid.order_id;

	-- Decrement stock atomically (based on order items)
	for v_item in
		select oi.product_id, oi.size, sum(oi.qty)::int as qty
		from public.fs_order_items oi
		where oi.order_id = fs_finalize_order_paid.order_id
...
revoke all on function public.fs_finalize_order_paid(uuid, text, text, text) from public;
grant execute on function public.fs_finalize_order_paid(uuid, text, text, text) to service_role;

-- Update checkout RPC to NOT decrement stock (stock is decremented on payment confirmation)
